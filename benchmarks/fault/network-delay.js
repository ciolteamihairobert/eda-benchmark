...
// Fault is injected via env vars on the service containers (FAULT_DELAY_MS / Fault__DelayMs).
// This script measures behaviour under the configured delay — set the containers before running.
export const options = {
  ...
  thresholds: {
    dotnet_duration:     ["p(95)<3000"],
    go_duration:         ["p(95)<3000"],
...
function makeOrder(vu, iter) {
  return JSON.stringify({
    orderId:    `ord-delay-${vu}-${iter}`,
    customerId: `cust-${vu}`,
    items: [{ sku: "SKU-003", quantity: 1, price: 14.99 }],
...
export function handleSummary(data) {
  ...
  console.log("\n=== Network-Delay Fault Results ===\n" + table);

  return {
    "benchmarks/results/network-delay-summary.json": JSON.stringify(data, null, 2),
